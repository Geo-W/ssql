...
#[async_trait]
pub trait Executable<'a>: Send + Sync {
    async fn execute(
        &self,
        conn: &'a mut tiberius::Client<Compat<TcpStream>>,
    ) -> SsqlResult<QueryStream<'a>>;
}

#[async_trait]
impl<'a, Ret, T> Executable<'a> for QueryBuilder<'a, Ret, T, NormalQuery>
where
...
#[async_trait]
impl<'a, Ret, Ta> Executable<'a> for QueryBuilder<'a, Ret, Ta, RawQuery>
where
...
pub struct QueryBuilder<'a, Ret, T: SsqlMarker, Stage = NormalQuery> {
    pub fields: HashMap<&'static str, Vec<&'static str>>,
    pub filters: Vec<String>,
    pub join: String,
    pub tables: HashSet<&'static str>,
    pub order: String,
    pub raw_sql: Option<String>,
    pub relation_func: fn(&str) -> &'static str,
    pub query_params: Vec<&'a dyn ToSql>,
    pub query_idx_counter: i32,
    pub func: Box<dyn Fn(&tiberius::Row) -> Ret + 'static + Send + Sync>,

    pub _marker: Option<PhantomData<T>>,
    pub _mark2: PhantomData<Stage>,
}

#[rustfmt::skip]
impl<'a, Ret, T, Stage: 'static> QueryBuilder<'a, Ret, T, Stage>
where
    T: SsqlMarker + 'static,
    QueryBuilder<'a, Ret, T, Stage>: Executable<'a>,
{
    async fn stream<F>(
        &mut self,
        conn: &'a mut tiberius::Client<Compat<TcpStream>>,
...
impl<'a, Ret, T> QueryBuilder<'a, Ret, T, NormalQuery>
where
    T: SsqlMarker,
{
    #[doc(hidden)]
    pub fn new<'b: 'a>(
        ...
        processing_fn: Box<dyn Fn(&tiberius::Row) -> Ret + 'static + Send + Sync>,
    ) -> QueryBuilder<'b, Ret, T>
    where
        T: SsqlMarker,
    {
        QueryBuilder {
...
pub trait SsqlMarker {
    ...
    fn raw_query<'a, Ret>(
        sql: &str,
        params: &[&'a dyn ToSql],
        process_fn: Box<dyn Fn(&tiberius::Row) -> Ret + 'static + Send + Sync>,
    ) -> QueryBuilder<'a, Ret, Self, RawQuery>
    where
        Self: Sized,
    {
